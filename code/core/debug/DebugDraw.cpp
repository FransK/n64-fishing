...
namespace Debug
{
    ...
    void DebugDraw::drawLine(const T3DVec3 &a, const T3DVec3 &b, color_t color) const
    {
        lines.push_back({a, b, color_to_packed16(color)});
    }

    void DebugDraw::drawBox(const Box3D &box) const
    {
    ...
    void DebugDraw::drawSphere(const T3DVec3 &center, float radius, color_t color) const
    {
        int steps = 12;
    ...
    void DebugDraw::draw(uint16_t *fb) const
    {
        if (lines.empty())
            return;
    ...
    void DebugDraw::printStart() const
    {
        rdpq_sync_pipe();
        rdpq_sync_tile();
        rdpq_sync_load();
    ...
    float DebugDraw::print(float x, float y, const char *str) const
    {
        int width = 8;
        int height = 8;
    ...
    float DebugDraw::printf(float x, float y, const char *fmt, ...) const
    {
        char buffer[128];
        va_list args;
        va_start(args, fmt);
